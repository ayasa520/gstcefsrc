...
#define DEFAULT_DEVICE_SCALE_FACTOR 1.0
...
static void
gst_cef_src_apply_browser_suspended_now(GstCefSrc *src, gboolean browser_suspended)
{
  ...
  if (frame)
  {
    const std::string script = browser_suspended
        ? R"JS(
            (() => {
              const playing = false;
        ...
        : R"JS(
            (() => {
              ...
              window.__hanabiSetPaused?.(false);
            })();
          )JS";
  ...
  params->SetString("state", browser_suspended ? "frozen" : "active");
  host->ExecuteDevToolsMethod(0, "Page.setWebLifecycleState", params);
  host->WasHidden(browser_suspended);
  if (!browser_suspended)
  {
...
static void
gst_cef_src_apply_browser_suspended(GstCefSrc *src, gboolean browser_suspended)
{
  if (!src)
    return;

  gst_cef_src_apply_browser_suspended_now(src, browser_suspended);
}
...
static inline guint
gst_cef_src_button_mask_for_button(gint button)
{
...
static inline int
gst_cef_src_get_mouse_modifiers(guint button_mask)
{
  int modifiers = 0;

  if (button_mask & gst_cef_src_button_mask_for_button(1))
    modifiers |= EVENTFLAG_LEFT_MOUSE_BUTTON;
  if (button_mask & gst_cef_src_button_mask_for_button(2))
    modifiers |= EVENTFLAG_MIDDLE_MOUSE_BUTTON;
  if (button_mask & gst_cef_src_button_mask_for_button(3))
    modifiers |= EVENTFLAG_RIGHT_MOUSE_BUTTON;

  return modifiers;
}

static inline cef_mouse_button_type_t
gst_cef_src_button_to_cef_button(gint button)
{
  switch (button)
  {
  case 2:
               ...
               CefRefPtr<Callback> callback) override
  {
    ...
    if (request == "ready")
    {
      ...
      else
      {
        ...
        error_msg << "error: (" << request << ") - " << "js ready signal sent with invalid cef state: " << cef_status;
    ...
    response << "{ " << "\"success\": \"" << (success ? "true" : "false") << "\", " << "\"cmd\": \"" << request << "\"" << " }";
    if (success)
    {
      GST_INFO_OBJECT(
...
{
public:
  RenderHandler(GstCefSrc *src) : src(src)
  {
  }
  ...
  void GetViewRect(CefRefPtr<CefBrowser> browser, CefRect &rect) override
  {
    ...
    GST_OBJECT_LOCK(src);
    const gdouble scale = src->device_scale_factor > 0.0 ? src->device_scale_factor : DEFAULT_DEVICE_SCALE_FACTOR;
    const gint width = src->vinfo.width ? MAX(1, (gint) lround((gdouble) src->vinfo.width / scale)) : DEFAULT_WIDTH;
    const gint height = src->vinfo.height ? MAX(1, (gint) lround((gdouble) src->vinfo.height / scale)) : DEFAULT_HEIGHT;
  ...
  {
    GST_LOG_OBJECT(src, "getting screen info");
    GST_OBJECT_LOCK(src);
    const gdouble scale = src->device_scale_factor > 0.0 ? src->device_scale_factor : DEFAULT_DEVICE_SCALE_FACTOR;
    const gint width = src->vinfo.width ? MAX(1, (gint) lround((gdouble) src->vinfo.width / scale)) : DEFAULT_WIDTH;
    const gint height = src->vinfo.height ? MAX(1, (gint) lround((gdouble) src->vinfo.height / scale)) : DEFAULT_HEIGHT;
...
{
public:
  AudioHandler(GstCefSrc *src) : src(src)
  {
  }
...
static void
gst_cef_src_send_mouse_move_on_ui(GstCefSrc *src, gint x, gint y, gboolean mouse_leave)
{
  CefRefPtr<CefBrowser> browser;
  guint button_mask = 0;
  ...
  g_mutex_lock(&src->state_lock);
  browser = src->browser;
  button_mask = src->pointer_button_mask;
  g_mutex_unlock(&src->state_lock);
  ...
  if (browser)
  {
    ...
    event.x = x;
    event.y = y;
    event.modifiers = gst_cef_src_get_mouse_modifiers(button_mask);
    browser->GetHost()->SendMouseMoveEvent(event, mouse_leave);
  }
}

static void
gst_cef_src_send_mouse_button_on_ui(GstCefSrc *src, gint x, gint y, gint button, gboolean mouse_up, gint click_count)
{
  CefRefPtr<CefBrowser> browser;
  guint button_mask = 0;
  ...
  g_mutex_lock(&src->state_lock);
  if (mouse_up)
    src->pointer_button_mask &= ~gst_cef_src_button_mask_for_button(button);
  else
    src->pointer_button_mask |= gst_cef_src_button_mask_for_button(button);
  button_mask = src->pointer_button_mask;
  browser = src->browser;
  ...
  if (browser)
  {
    ...
    browser->GetHost()->SendMouseClickEvent(
        event,
        gst_cef_src_button_to_cef_button(button),
        mouse_up,
...
static void
gst_cef_src_send_mouse_wheel_on_ui(GstCefSrc *src, gint x, gint y, gint delta_x, gint delta_y)
{
  CefRefPtr<CefBrowser> browser;
  guint button_mask = 0;
...
static void
gst_cef_src_mouse_move(GstCefSrc *src, gint x, gint y, gboolean mouse_leave)
{
  if (!src)
    return;

  gst_cef_src_send_mouse_move_on_ui(src, x, y, mouse_leave);
}

static void
gst_cef_src_mouse_button(GstCefSrc *src, gint x, gint y, gint button, gboolean mouse_up, gint click_count)
{
  if (!src)
    return;

  gst_cef_src_send_mouse_button_on_ui(src, x, y, button, mouse_up, click_count);
}

static void
gst_cef_src_mouse_wheel(GstCefSrc *src, gint x, gint y, gint delta_x, gint delta_y)
{
  if (!src)
    return;

  gst_cef_src_send_mouse_wheel_on_ui(src, x, y, delta_x, delta_y);
}

static void
gst_cef_src_audio_frame(GstCefSrc *src, const gchar *payload)
{
  CefRefPtr<CefBrowser> browser;
  CefRefPtr<CefFrame> frame;
  ...
  if (browser)
  {
    ...
    if (frame)
    {
      const std::string script =
          "window.__hanabiApplyAudioFrame?.(" + std::string(payload) + ");";
      frame->ExecuteJavaScript(script, frame->GetURL(), 0);
    }
  }
}
...
public:
  ...
  void OnAfterCreated(CefRefPtr<CefBrowser> browser) override
  {
    ...
    gst_cef_src_apply_browser_suspended_now(src, src->browser_suspended);
                         ...
                         GParamSpec *pspec)
{
  ...
  {
    src->device_scale_factor = g_value_get_double(value);
    break;
  }
  case PROP_BROWSER_SUSPENDED:
  {
    src->browser_suspended = g_value_get_boolean(value);
    gst_cef_src_apply_browser_suspended(src, src->browser_suspended);
    break;
  }
  case PROP_JS_FLAGS:
  {
                         ...
                         GParamSpec *pspec)
{
  ...
  case PROP_LISTEN_FOR_JS_SIGNAL:
    ...
    break;
  case PROP_DEVICE_SCALE_FACTOR:
    g_value_set_double(value, src->device_scale_factor);
    break;
  case PROP_BROWSER_SUSPENDED:
    g_value_set_boolean(value, src->browser_suspended);
    break;
  case PROP_JS_FLAGS:
...
gst_cef_src_init(GstCefSrc *src)
{
  ...
  src->device_scale_factor = DEFAULT_DEVICE_SCALE_FACTOR;
  src->browser_suspended = FALSE;
  src->pointer_button_mask = 0;
  src->js_flags = NULL;
...
gst_cef_src_class_init(GstCefSrcClass *klass)
{
  ...
  klass->mouse_move = gst_cef_src_mouse_move;
  klass->mouse_button = gst_cef_src_mouse_button;
  klass->mouse_wheel = gst_cef_src_mouse_wheel;
  klass->audio_frame = gst_cef_src_audio_frame;
  ...
  g_object_class_install_property(gobject_class, PROP_DEVICE_SCALE_FACTOR,
                                  g_param_spec_double("device-scale-factor", "device-scale-factor",
                                                      "Device scale factor used for windowless rendering",
                                                      0.1, 16.0,
                                                      DEFAULT_DEVICE_SCALE_FACTOR, (GParamFlags)(G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS | GST_PARAM_MUTABLE_READY)));

  g_object_class_install_property(gobject_class, PROP_BROWSER_SUSPENDED,
                                  g_param_spec_boolean("browser-suspended", "browser-suspended",
                                                       "Suspend Chromium lifecycle and rendering while preserving the last frame",
                                                       FALSE, (GParamFlags)(G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS | GST_PARAM_MUTABLE_PLAYING)));
  ...
  gst_cef_src_signals[SIGNAL_MOUSE_MOVE] =
      g_signal_new("mouse-move",
                   G_TYPE_FROM_CLASS(klass),
                   (GSignalFlags)(G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION),
                   G_STRUCT_OFFSET(GstCefSrcClass, mouse_move),
                   NULL, NULL, g_cclosure_marshal_generic,
                   G_TYPE_NONE, 3,
                   G_TYPE_INT, G_TYPE_INT, G_TYPE_BOOLEAN);

  gst_cef_src_signals[SIGNAL_MOUSE_BUTTON] =
      g_signal_new("mouse-button",
                   G_TYPE_FROM_CLASS(klass),
                   (GSignalFlags)(G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION),
                   G_STRUCT_OFFSET(GstCefSrcClass, mouse_button),
                   NULL, NULL, g_cclosure_marshal_generic,
                   G_TYPE_NONE, 5,
                   G_TYPE_INT, G_TYPE_INT, G_TYPE_INT, G_TYPE_BOOLEAN, G_TYPE_INT);

  gst_cef_src_signals[SIGNAL_MOUSE_WHEEL] =
      g_signal_new("mouse-wheel",
                   G_TYPE_FROM_CLASS(klass),
                   (GSignalFlags)(G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION),
                   G_STRUCT_OFFSET(GstCefSrcClass, mouse_wheel),
                   NULL, NULL, g_cclosure_marshal_generic,
                   G_TYPE_NONE, 4,
                   G_TYPE_INT, G_TYPE_INT, G_TYPE_INT, G_TYPE_INT);

  gst_cef_src_signals[SIGNAL_AUDIO_FRAME] =
      g_signal_new("audio-frame",
                   G_TYPE_FROM_CLASS(klass),
                   (GSignalFlags)(G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION),
                   G_STRUCT_OFFSET(GstCefSrcClass, audio_frame),
                   NULL, NULL, g_cclosure_marshal_generic,
                   G_TYPE_NONE, 1,
                   G_TYPE_STRING);
